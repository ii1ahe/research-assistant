...
% ===== CONFIGURATION =================================================
% Descriptive team label; members are listed by GitHub handle.
...
\newcommand{\reportdate}{2026-09-18}
...
\texttt{gemini-3.8-flash}, with Tavily for web search and key-free Wikipedia and arXiv for the
...
that classification is the last place the \emph{textual retry hint} exists --- so that is where
it is now parsed. An HTTP \texttt{Retry-After} header without the hint in the exception message
is not read. A new \texttt{UpstreamRateLimitError} carries the parsed delay (a number,
...
\texttt{validate\_answer} handles the \emph{model's} output and checks citation structure. The
current check also accepts an empty answer with no citations; that is an input-contract gap, not
a successful synthesis. Neither check establishes that a source supports a claim
...
\begin{itemize}[leftmargin=1.4em,itemsep=0.15em]
  \item \textbf{Keyed on the canonical query}, not the raw one --- two questions differing only
        in case or whitespace share an entry, which makes a re-run cheap.
...
6\,375 lines of test code against 4\,681 lines of application code. Coverage is measured over
the application package only, and reported because it is real rather than a target: two modules
sit below 100\,\% deliberately, each missing only lines that execute when a third-party library
misbehaves in ways we cannot provoke.
...
Two testing habits changed the code. First, \emph{tests assert
...
against a real PostgreSQL with \textbf{zero skips}; the demo answers end to end, writing a real
session row from inside the container. The README records the exact commands and the date
they were verified.
...
\texttt{PER\_SOURCE\_TIMEOUT\_SECONDS} & 10.0 & $>0$ & Fetch deadline, retries included; cache waits excluded. \\
...
\begin{enumerate}[leftmargin=1.6em,itemsep=0.15em]
  ...
  \item \textbf{The retry policy reacts to throttles but cannot anticipate them.} Measured: the
        ...
        three attempts landed inside the same window. Honouring the textual retry hint
...
HTTP API. \texttt{docs/security.md} records the security posture and the gaps, checked against the
...
OpenAI Codex & Final documentation audit and consistency corrections; source and build outputs checked. \\
\bottomrule
\end{tabularx}
\end{table}
...
The full procedure is in \texttt{README.md}. The default benchmark makes ten measured
synthesis calls and two warm-up calls, plus any retries; it needs live credentials
and can consume provider quota. This version has no retrieval-only mode.
