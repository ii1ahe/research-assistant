...
% ===== CONFIGURATION =================================================
% Fill \teamname. Members are listed by GitHub handle. Everything else is
% final.
\newcommand{\teamname}{\phh{team name}}
...
Our retry policy then waited a full-jitter backoff of at most 0.5\,s between three attempts, so
every attempt landed inside the same throttle window: a throttle the provider described as
recoverable in 22 seconds became a lost question that failed in under a second, with three
attempts spent and a 30-second deadline unused. The delay the provider asked for was roughly
forty times our backoff ceiling.
...
that classification is the last place the \texttt{Retry-After} value exists --- so that is where
it is now parsed. A new \texttt{UpstreamRateLimitError} carries the parsed delay (a number,
...
instead by the operation's deadline. A throttle that names no parseable delay is still
classified, and still retried on the ordinary backoff. The payload itself still never reaches a
log or a user --- a test asserts that the measured payload's text does not appear in the message.
...
\texttt{pytest}, 357 tests, 96\,\% statement coverage over \texttt{researcher/},
6\,347 lines of test code against 4\,615 lines of application code. Coverage is measured over
...
credentials, no network and no quota consumed; the full 357-test suite passes inside Compose
...
is recorded in \texttt{artefacts/reproduction-codespaces-bfec78.txt}: 349 passed with the seven
