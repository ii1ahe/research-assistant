...
% ===== CONFIGURATION =================================================
% Exactly one thing here is still unfilled: \teamname and the member
% name. Everything else is final.
\newcommand{\teamname}{}
...
\titlespacing*{\subsection}{0pt}{1.1ex plus .2ex minus .1ex}{0.5ex}
...
Each source is an independent task with its own deadline, run under an
\texttt{asyncio.Semaphore}, so a slow or dead source costs one citation rather than the whole
run; the answer is synthesised once through the supplied AI module, validated structurally, and
stored in PostgreSQL with a cache of retrieval results.
...
The scope boundary is set by what the system refuses to do, which is the more interesting list.
It has \textbf{no HTTP API} (ADR-006) --- the deliverable is a CLI program and the container
therefore publishes no port; an API would add an authentication surface, a story for simultaneous
clients, and a lifecycle none of which this topic asks for. It makes \textbf{no answer-quality
claim}: it verifies that an answer is \emph{internally consistent}, never that its sources
support its claims (Section~\ref{sec:limits}). And it offers \textbf{no second opinion} --- one
provider, one model --- because an ensemble would triple the quota cost of a demo run, which on a
free tier is the binding constraint (Section~\ref{sec:politeness}).

The inputs are a question, a set of source names and a results-per-source limit; the output is a
rendered answer, a numbered source list, a per-source retrieval report, and a process exit
status. The last is load-bearing: the program distinguishes ``answered'' from ``answered but
incomplete'' from ``no usable answer'' from ``you invoked it wrong'', because a script wrapping
this tool needs that distinction and a single non-zero code would erase it.
Table~\ref{tab:exitcodes} specifies it.
...
\texttt{gemini-3.8-flash}, with Tavily for web search and the key-free Wikipedia and arXiv
endpoints for the rest. Two facts about that choice are worth recording because they cost us
time. First, the providers in \texttt{ai/providers/google.py} fall back to
\texttt{gemini-2.0-flash}, which is retired --- every 2.x model returns HTTP~404, so the
application configures the model explicitly rather than relying on the supplied default.
Second, Gemini's 3.x models emit \emph{reasoning} tokens that count against the same
\texttt{max\_output\_tokens} budget as the answer, so a small token cap yields an empty or
truncated completion rather than an error; the synthesis budget has to accommodate the reasoning
...
Multi-word queries match nothing at all, so no query-rewriting heuristic recovers this --- a
keyword-derivation scheme was our first idea and it does not work either, because
\texttt{photosynthesis main stages} already is the keyword derivation. Since \texttt{ai/} is
...
flat layout also means the stock \texttt{python:3.14.7-slim} image works with a plain
\texttt{COPY} and a \texttt{WORKDIR}, with no \texttt{pip install .} and no packaging step in the
container. The application is 4\,494 lines across 24 modules; the layering rule is enforced by
import direction rather than by tooling, so a lower layer never imports from a higher one.
...
\texttt{validation.py} & 358 & Normalising input; validating a synthesised answer. \\
\texttt{config.py} & 316 & Typed settings, derived flags, safe error messages. \\
\texttt{errors.py} & 142 & The exception taxonomy and the exit-status mapping. \\
\texttt{services/orchestrator.py} & 240 & Bounded concurrent retrieval; one outcome per source. \\
\texttt{services/ai\_service.py} & 467 & The only place \texttt{ai/} is called; timeouts, retries, translation. \\
\texttt{services/cache.py} & 185 & Cache-aside reads and writes around the storage seam. \\
\texttt{services/resilience.py} & 201 & The retry policy and its backoff schedule. \\
\texttt{storage/interfaces.py} & 146 & The two \texttt{Protocol}s and the error contract. \\
\texttt{storage/postgres.py} & 253 & \texttt{asyncpg} implementation; migrations applied at startup. \\
...
\texttt{bootstrap.py} & 266 & The composition root --- the only module naming concrete classes. \\
\texttt{cli.py}, \texttt{rendering.py} & 530 & Argument parsing, exit statuses, terminal rendering. \\
\bottomrule
\end{tabularx}
\end{table}
...
classes, and the code depends on the protocol rather than on a concrete implementation. We chose
protocols over abstract base classes deliberately and for a testability reason: an
...
Frozen models give us immutability for free, and \texttt{extra="forbid"} means a typo in a
keyword argument is an error at construction rather than a silently ignored field --- which, in
a system whose failure modes are mostly ``a source quietly returned nothing'', is worth a great
deal. One deliberate exception is \texttt{AnswerWithCitations}, which is the supplied module's
...
timeouts retried for fetches but not for synthesis, where a retry cannot help --- the deadline
wraps the whole call, so a retried synthesis is guaranteed to fail again). And
\texttt{SourceOutcome} models a per-source retrieval as a value that carries either the sources
or the reason it failed; the orchestrator's \texttt{gather} therefore never raises for an
expected failure, and ``source X is unavailable'' becomes data the renderer can print rather
than an exception that unwinds the run.
...
name, a question that is empty or over \texttt{max\_question\_length}, or a missing credential
for the selected provider is rejected here with exit~2. \texttt{bootstrap.py} then builds the
...
--- the tasks are I/O-bound and the parallelism is data-independent, so threads would add
preemption and locking for no benefit. The whole application is single-threaded.
...
overhead and the cache lookups preceding each fetch. A mix with more even latencies would show a
larger ratio and a mix with one dominant source almost none, which is why we report the ceiling
beside the number.
...
provider's quota is and it does not react to being throttled. We treat that as a known gap
rather than as a solved problem, and running the container gave us a measurement of it.

The free tier for the configured model permits \textbf{20 generation requests per day, per
model}, and a demo run costs five syntheses --- so three runs exhaust the model's bucket. When
the bucket ran out, the provider answered with a payload that named both the limit and the
remedy:

\begin{lstlisting}[style=python,numbers=none,caption={The provider's own words, abridged. The
hint is precise and our policy ignores all of it.},captionpos=b]
429 RESOURCE_EXHAUSTED ... limit: 20, model: gemini-3.8-flash
                          Please retry in 21.9s
\end{lstlisting}

Our retry policy waits a full-jitter backoff of at most 0.5\,s between three attempts. Every
attempt therefore lands inside the same throttle window, and a throttle that the provider
describes as recoverable in 22 seconds is converted into a lost question that fails in under a
second with three attempts and a 30-second deadline unused. The delay the provider asks for is
roughly forty times our backoff ceiling.

Honouring it is not a one-line change, and the reason is a decision we would defend separately:
...
that classification is the last place the \texttt{Retry-After} value exists, and our taxonomy
carries a category but has nowhere to put a delay. Fixing it properly means a retryable error
type that carries the delay, raised where the payload is still in scope, plus a policy that waits
for the larger of its own backoff and the provider's instruction. The current behaviour is not
unsafe --- a throttled run degrades to a disclosed partial answer --- but it turns a recoverable
...
so no other layer has to know what a provider exception looks like. Four things happen there.
...
\paragraph{Timeouts with per-call-type retry semantics.} Per-source fetches are bounded by
...
\begin{enumerate}[leftmargin=1.6em,itemsep=0.15em]
  ...
  \item The run continues. The other three questions answered normally, the CLI printed which
        ones failed, and the process exited~1 --- the exit status distinguishing ``some questions
...
``the program broke''. What went wrong is entirely in steps 3--5: a condition the provider
described as recoverable in 22 seconds was treated as unrecoverable in one, because our
classification is one-dimensional --- it asks whether a failure is the \emph{kind} that can be
retried, never \emph{when}.
...
\texttt{validate\_answer} handles the \emph{model's} output and is the mirror image: it assumes
the model may be wrong and checks the structure it was supposed to produce. Together the two
validate everything crossing a trust boundary in either direction. What we deliberately do
\emph{not} do is validate that a source supports a claim attached to it --- the core limitation,
and Section~\ref{sec:limits}.
...
can \emph{see} what is cached and why a given lookup missed, which a digest column would hide.
The write path is \texttt{INSERT \ldots ON CONFLICT} against exactly that key, so the upsert is
...
Migrations are discovered from the filesystem, recorded by filename with a checksum, and that
checksum is verified on every later start --- so they are append-only, and editing a file that
has already been applied is refused rather than silently skipped. Every statement is idempotent,
so a partially applied migration can be re-run safely. The schema is applied automatically at
...
\begin{itemize}[leftmargin=1.4em,itemsep=0.15em]
  \item \textbf{Keyed on the canonical query}, not the raw one --- so two questions differing
        only in case or whitespace share an entry, which is what makes a re-run cheap.
  ...
  \item \textbf{A hit reports \texttt{attempts=0}}, which distinguishes ``this came from the
        cache'' from ``this succeeded on the first try'' in the per-source report. Without that
        distinction a cached run and a fast network look identical in the output.
...
during this phase. The lesson generalises: a flag that can be set inconsistently with the state
it describes is a flag that will be.
\end{findingbox}
...
\texttt{pytest}, 327 tests, 96\,\% statement coverage over \texttt{researcher/},
5\,925 lines of test code against 4\,494 lines of application code. Coverage is measured over
the application package only, and the figure is reported because it is real rather than because
it is a target: two modules sit below 100\,\% deliberately, and the reason is always a line that
can only execute when a third-party library misbehaves in a way we cannot provoke.
...
they passed while quietly reaching the live API. Those tests were green and wrong, which is the
worst state a test can be in. Private ranges are permitted because the PostgreSQL tests need
...
Two testing habits are worth naming because they changed the code. First, \emph{tests assert
refusals as often as successes}: the public-error-message test plants a fake API key in the
environment, forces a validation failure, and asserts the key does \emph{not} appear in the
resulting message. Second, \emph{a test that cannot fail is a liability}, and the correct
response to one is to raise its cost of failure rather than delete it --- which is why the
...
ownership of \texttt{/app}, and drops to it. There is no \texttt{EXPOSE} and no published port,
which is a direct consequence of ADR-006 --- this project has no HTTP API --- and is called out
in a comment in the file so that its absence reads as a decision rather than an oversight. The
...
\begin{enumerate}[leftmargin=1.6em,itemsep=0.2em]
  ...
  \item \textbf{The offline guard refused Compose's database.} The guard permitted loopback as a
        ...
        link-local ranges and refusing public ones --- which is the rule we had meant to write
        the first time.
...
credentials, no network and no quota consumed; the full 327-test suite passes inside Compose
against a real PostgreSQL with \textbf{zero skips}; and the demo answers end to end, writing a
real session row from inside the container. The README records the exact commands and the date
they were verified.
...
string'' --- without it the value arrives as \texttt{''}, which is falsy but not \texttt{None},
and every \texttt{is None} check downstream would wrongly conclude a credential is present. And
...
\texttt{PER\_SOURCE\_TIMEOUT\_SECONDS} & 10.0 & $>0$ & Deadline per fetch, and per retry attempt. \\
\texttt{SYNTHESIS\_TIMEOUT\_SECONDS} & 30.0 & $>0$ & Deadline for one synthesis, retries included. \\
\texttt{MAX\_RESULTS\_PER\_SOURCE} & 3 & 1--20 & Results requested per source. \\
...
\texttt{PERSIST\_SESSIONS} & \texttt{true} & bool & Set false to run without touching the database. \\
\bottomrule
\end{tabularx}
\end{table}

The two timeout settings are separate for a reason that a defect made concrete. They were
originally one knob, and sharing it made two different concerns invisible: raising the fetch
budget to accommodate a slow search provider silently bought synthesis time as well, and
lowering it to fail fast on a dead host silently began killing synthesis. Measured against the
configured model, synthesis ran at 2.9\,s, 3.9\,s, 6.1\,s and 7.6\,s on a quiet run and then hit
the shared ten-second ceiling --- roughly one synthesis in six refused on a machine doing nothing
else. Two settings, two deadlines.
...
\begin{enumerate}[leftmargin=1.6em,itemsep=0.3em]
  ...
  \item \textbf{The retry policy cannot recover from a rate limit.} Measured: the provider asked
        for 21.9\,s while the policy waits a jittered $\leq$0.5\,s, so all three attempts land
        inside the same window. Section~\ref{sec:politeness} sets out what a fix requires --- a
        \texttt{Retry-After}-carrying error raised where the payload still exists, and a policy
        that waits for the larger of its backoff and the provider's instruction.
        \emph{Future work:} that, plus a token bucket per provider --- the current bound reacts to
        nothing.
  ...
  \item \textbf{Retrieved web content is untrusted input fed to a language model.} Snippets come
        ...
        can carry text aimed at the model rather than the reader. We do structural work only: the
        prompt instructs the model to answer from the numbered sources and not to invent them,
        and we validate the shape of the result. We cannot validate that a source supports the
        claim attached to it. \emph{Future work:} delimit retrieved text unambiguously as data,
        strip instruction-shaped markup, and add an entailment check.
  ...
  \item \textbf{PostgreSQL is configured with \texttt{trust} authentication.} \texttt{pg\_hba.conf}
        ...
        is bounded rather than accidental --- the server listens only on loopback and the
        application role is not a superuser --- but the exposure appears the moment
        \texttt{listen\_addresses} is widened or the port is forwarded.
  ...
  \item \textbf{Dependencies are pinned but not scanned.} Pinning gives reproducible builds;
        nothing checks those pins against known vulnerabilities. \emph{Future work:}
        \texttt{pip-audit} in CI plus automated update pull requests.
\end{enumerate}
...
The Architecture Decision Records in \texttt{docs/adr/} carry the reasoning behind the choices
this report argues for: the flat layout, PostgreSQL for both persistence concerns, Protocol-based
storage interfaces, the bounded-concurrency model, and the decision not to build an HTTP API.
\texttt{docs/security.md} records the security posture and the gaps, checked against the running
...
The AI assistants were used as a tool and not as an authority. The defects in this report were
found by executing the system --- building the image, running the suite against a real database,
and benchmarking live APIs --- and each fix was verified by a test that fails without it. All
architectural decisions, and any error remaining in this document, are the author's.
...
The full procedure is in \texttt{README.md}; this is the short form.

\begin{lstlisting}[style=python,numbers=none,caption={Reproducing Table~\ref{tab:bench} from a
clean checkout.},captionpos=b]
# 1. Dependencies and a database
python -m venv .venv && . .venv/bin/activate
pip install -r requirements.txt -r requirements-dev.txt
cp .env.example .env        # then edit .env: LLM_PROVIDER=gemini, LLM_MODEL, keys

# 2. Retrieve (no model calls, so no quota is spent) -- the
#    1.67x retrieval row of Table 4 comes from this mode.
python scripts/bench.py --mode retrieval --repeat 1

# 3. Full run (5 questions x 2 sweeps x 1 synthesis = 10 model calls)
python scripts/bench.py --repeat 1

# 4. Both modes write artefacts/bench.json and print a report. A run
#    that could not answer every question in both sweeps withholds
#    the synthesis and end-to-end rows and exits 1, by design.
\end{lstlisting}

Cost note, because it determines whether the numbers reproduce at all: the free tier allows
\textbf{20 generation requests per day, per model}, and the full benchmark costs ten. Running
step~2 first is therefore not just cheaper --- it is what lets the retrieval row be re-measured
after the day's quota is spent, since retrieval consumes no quota at all.
